...
class EditUser extends Component {
  ...
  async componentDidMount() {
    ...
    const updateUser = await axios(`http://localhost:5000/api/users/${id}`);
  ...
  updateUserHandler = async (e) => {
    e.preventDefault();
    try {
      const user = await axios.put(`http://localhost:5000/api/users/${this.state.id}`, {
      ...
      <div className="Edit-User-Wrapper">
        ...
        <form onSubmit={this.updateUserHandler}>
          ...
          <input
            ...
            id="age"
          />
          <button type="submit" className="Edit-User-Submit fa fa-pencil">Edit</button>
        </form>
